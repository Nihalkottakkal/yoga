  ...
  return (
    ...
    >
      ...
      {rings === 4 ? (
        ...
        icon && (
          <i
            className={`fas ${icon} text-2xl relative z-10 text-${rings === 1 ? "goldLight" : rings === 2 ? "sage" : "sand"}`}
          />
        )
      )}
    </div>
...
export default function Philosophy() {
  return (
    <section
      id="philosophy"
      className="py-24 md:py-32 bg-sand text-white relative"
    >
      {/* <div
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <Reveal className="text-center mb-20">
          ...
          <h2 className="text-4xl md:text-5xl text-charcoal font-serif mb-6">
            Yogic Science of Evolvement
          </h2>
          <p className="max-w-2xl mx-auto text-earth/70 font-light">
            Understanding the multi-dimensional nature of human existence
            through the Pancha Koshas (Five Sheaths).
          </p>
        </Reveal>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {KOSHAS.map((k, i) => (
            <Reveal
              key={k.name}
              delay={i * 100}
              className={`kosha-card bg-charcoal glass-panel-dark p-8 rounded-2xl text-center border-t-4 ${k.borderColor}`}
            >
              <KoshaRing rings={k.rings} icon={k.icon} glow={k.glow} />
              <h3 className={`font-serif text-xl mb-2 ${k.textColor}`}>
                {k.name}
              </h3>
